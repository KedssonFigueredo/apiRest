...
app.get('/books_lista/:book_id',
    (req, res) => {
        const { book_id } = req.params;
        const book = books.find((book) => book.id === book_id);
        if (!book) { res.status(404).json("nenhum livro encontrado"); }
        else {
            return res.status(200).json(book);
        }
    });
...
//update um livros específico 
app.put('/books_update/:book_id', (req, res) => {
    const { titulo, autor, publicacao } = req.body;
    const { book_id } = req.params;
    const book = books.find(book => book.id === book_id);
    book.id = book.id; book.titulo = titulo ? titulo : book.titulo;
